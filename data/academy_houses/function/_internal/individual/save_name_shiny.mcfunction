# Sauvegarder le nom du joueur pour les Pokémon shiny
...
$execute store result storage academy_houses:temp score int 1 run scoreboard players get @s shiny_caught
$data modify storage academy_houses:rankings shiny.rank$(rank) set value {name:"",score:0}
$data modify storage academy_houses:rankings shiny.rank$(rank).name set from storage academy_houses:temp current_name
$data modify storage academy_houses:rankings shiny.rank$(rank).score set from storage academy_houses:temp score